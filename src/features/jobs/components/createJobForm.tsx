  ...
  const onSubmit = (data: CreateJobData) => {
    createJobMutation.mutate(data, {
      onSuccess: () => {
        ...
        form.reset();
        navigate("/recruiter/jobs"); // Redirect to recruiter jobs dashboard
      },
      onError: (error: any) => {
      ...
      <Card className="border border-slate-100 shadow-xl shadow-slate-100/50 rounded-3xl dark:border-slate-800 dark:shadow-none bg-card">
        <CardHeader className="border-b border-slate-100 dark:border-slate-800 pb-5">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <CardTitle className="text-xl font-bold tracking-tight">Create Job Posting</CardTitle>
        ...
        <CardContent className="pt-6">
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            >
              {createJobMutation.isPending ? "Creating Job..." : "Publish Job Listing"}
            </Button>
          </form>
        </CardContent>
      </Card>
